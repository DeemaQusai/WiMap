...
public class EstimateAP {
	...
	public static void estimateAP(MAC_samples ap_readings)
	{
		...
		if (ap_readings.getSampleArr().size() < 10)
		{
			//the Maximum signal level
			X= ap_readings.getSampleArr().get(0).getX();
			Y= ap_readings.getSampleArr().get(0).getY();

			//draw circle around the AP (Estimation) 
			g.setColor(Color.BLACK); 
			g.drawOval((int)(X-60), (int)(Y-60), 120 , 120);
			
		} else if (ap_readings.getSampleArr().size() >= 10)
		{
			Max = (int)(0.2 * ap_readings.getSampleArr().size()); 

			for ( int j = 0  ; j < Max; j++)
			{ 
				sum1 = sum1 + ap_readings.getSampleArr().get(j).getX();
				sum2 = sum2 + ap_readings.getSampleArr().get(j).getY();
			}
